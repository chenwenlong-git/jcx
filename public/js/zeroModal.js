...
(function(obj) {
    ...
    function _buildModal(opt) {
        ...
        if(opt.title==false){
		    ...
		    var _container = $('<div zero-unique-container="' + opt.unique + '" class="zeromodal-container" style="z-index:' + _tmp_last_zindex + ';width:' + _width + 'px;  min-width:'+ _min_width +'; height:' + _height + 'px;left:' + _left + 'px;top:' + (opt.transition ? _top - 50 : _top) + 'px"></div>');
		 }
        ...
        //// 构建头部
        var _headerHtml = '<div class="zeromodal-bg"><div zeromodal-unqiue-header="' + opt.unique + '" class="zeromodal-header">';
        _headerHtml += '        <div title="关闭" zero-close-unique="' + opt.unique + '" class="zeromodal-close"><img src="../../public/images/close-w.png"></div>';
        if (opt.max) {
        ...
        if (!opt.url) {
            ...
            if (opt.iframe) {
                var _iframe = $('<iframe src="' + opt.url + '" class="zeromodal-frame" id="myIframe"  name="myIframe" ></iframe>');
    ...
    function _buildFooter(opt, _container) {
        if (opt.ok || opt.cancel || (opt.buttons !== undefined && opt.buttons.length > 0)) {
			...
			if(opt.cancel==false){
              _footer += '        <div zeromodal-btn-container="' + opt.unique + '" class="zeromodal-btn-container" style="padding-right:25px;"></div>';
			}else{
